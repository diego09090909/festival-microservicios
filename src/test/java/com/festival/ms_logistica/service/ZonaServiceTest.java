...
class ZonaServiceTest {
    ...
    @Test
    @DisplayName("crearZona debe guardar la zona cuando el evento existe y esta PUBLICADO")
    void crearZonaCuandoEventoValidoDebeGuardar() {

        when(eventoClient.obtenerEvento(1L)).thenReturn(eventoPublicado);
        when(zonaMapper.toEntity(dto)).thenReturn(zonaEntidad);
        when(zonaRepository.save(any(Zona.class))).thenReturn(zonaEntidad);

        ZonaDTO esperado = new ZonaDTO();
        esperado.setId(1L);
        esperado.setNombre("Zona Primeros Auxilios");
        when(zonaMapper.toDTO(zonaEntidad)).thenReturn(esperado);

        ZonaDTO resultado = zonaService.crearZona(dto);

        assertNotNull(resultado);
        assertEquals(1L, resultado.getId());
        assertEquals("Zona Primeros Auxilios", resultado.getNombre());

        verify(zonaRepository, times(1)).save(any(Zona.class));
    }

    @Test
    @DisplayName("crearZona debe lanzar ResourceNotFoundException cuando el evento no existe")
    void crearZonaCuandoEventoNoExisteDebeLanzarExcepcion() {
    ...
    @Test
    @DisplayName("crearZona debe lanzar ResourceNotFoundException cuando el evento no esta PUBLICADO")
    void crearZonaCuandoEventoNoPublicadoDebeLanzarExcepcion() {
        ...
        assertThrows(ResourceNotFoundException.class, () -> zonaService.crearZona(dto));

        verify(zonaRepository, never()).save(any(Zona.class));
    }

    @Test
    @DisplayName("actualizarZona debe actualizar nombre y tipo cuando la zona existe")
    void actualizarZonaCuandoExisteDebeActualizar() {

        ZonaDTO dtoActualizado = ZonaTestDataFactory.zonaDtoFalsa();
        dtoActualizado.setNombre("Zona Primeros Auxilios Renovada");
        ...
        ZonaDTO esperado = new ZonaDTO();
        esperado.setId(1L);
        esperado.setNombre("Zona Primeros Auxilios Renovada");
        when(zonaMapper.toDTO(zonaEntidad)).thenReturn(esperado);

        ZonaDTO resultado = zonaService.actualizarZona(1L, dtoActualizado);

        assertEquals("Zona Primeros Auxilios Renovada", resultado.getNombre());
    ...
    @Test
    @DisplayName("actualizarZona debe lanzar ResourceNotFoundException cuando la zona no existe")
    void actualizarZonaCuandoNoExisteDebeLanzarExcepcion() {

        when(zonaRepository.findById(99L)).thenReturn(Optional.empty());

        assertThrows(ResourceNotFoundException.class,
                () -> zonaService.actualizarZona(99L, dto));
