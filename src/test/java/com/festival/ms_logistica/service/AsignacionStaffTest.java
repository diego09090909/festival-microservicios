...
class AsignacionStaffServiceTest {
    ...
    @Test
    @DisplayName("ListaStaffPorZona debe retornar la lista cuando existe staff asignado")
    void listaStaffPorZonaConDatosDebeRetornarLista() {

        when(asignacionStaffRepository.findByZonaId(1L)).thenReturn(List.of(asignacionEntidad));

        AsignacionStaffDTO dtoEsperado = new AsignacionStaffDTO();
        dtoEsperado.setId(1L);
        when(asignacionStaffMapper.toDTO(asignacionEntidad)).thenReturn(dtoEsperado);

        List<AsignacionStaffDTO> resultado = asignacionStaffService.ListaStaffPorZona(1L);
    ...
    @Test
    @DisplayName("ListaStaffPorZona debe lanzar excepcion cuando no hay staff en la zona")
    void listaStaffPorZonaSinDatosDebeLanzarExcepcion() {

        when(asignacionStaffRepository.findByZonaId(99L)).thenReturn(List.of());

        assertThrows(ResourceNotFoundException.class,
                () -> asignacionStaffService.ListaStaffPorZona(99L));
    }

    @Test
    @DisplayName("listaStaffPorEvento debe retornar la lista cuando existe staff en el evento")
    void listaStaffPorEventoConDatosDebeRetornarLista() {

        when(asignacionStaffRepository.findByZona_EventoId(1L)).thenReturn(List.of(asignacionEntidad));

        AsignacionStaffDTO dtoEsperado = new AsignacionStaffDTO();
        dtoEsperado.setId(1L);
        when(asignacionStaffMapper.toDTO(asignacionEntidad)).thenReturn(dtoEsperado);

        List<AsignacionStaffDTO> resultado = asignacionStaffService.listaStaffPorEvento(1L);
    ...
    @Test
    @DisplayName("listaStaffPorEvento debe lanzar excepcion cuando no hay staff en el evento")
    void listaStaffPorEventoSinDatosDebeLanzarExcepcion() {

        when(asignacionStaffRepository.findByZona_EventoId(99L)).thenReturn(List.of());

        assertThrows(ResourceNotFoundException.class,
                () -> asignacionStaffService.listaStaffPorEvento(99L));
    }

    @Test
    @DisplayName("eliminarAsignacion debe eliminar cuando la asignacion existe")
    void eliminarAsignacionCuandoExisteDebeEliminar() {

        when(asignacionStaffRepository.findById(1L)).thenReturn(Optional.of(asignacionEntidad));

        asignacionStaffService.eliminarAsignacion(1L);

        verify(asignacionStaffRepository, times(1)).deleteById(1L);
    }

    @Test
    @DisplayName("eliminarAsignacion debe lanzar excepcion cuando la asignacion no existe")
    void eliminarAsignacionCuandoNoExisteDebeLanzarExcepcion() {

        when(asignacionStaffRepository.findById(99L)).thenReturn(Optional.empty());

        assertThrows(ResourceNotFoundException.class,
                () -> asignacionStaffService.eliminarAsignacion(99L));

        verify(asignacionStaffRepository, never()).deleteById(any());
    }
}