...
public class AsignacionStaffService {
    ...
    public AsignacionStaffDTO actualizarAsignacion(Long usuarioId, AsignacionStaffDTO dto){
        ...
        if (!staff.getUsuarioId().equals(dto.getUsuarioId())) {
            log.warn("El usuarioId almacenado {} no coincide con el usuarioId ingresado {}", staff.getUsuarioId(), dto.getUsuarioId());
        ...
        AsignacionStaff staffActulizado =  asignacionStaffRepository.save(staff);
        ...
        notificacioneClient.crearNotificacion(new NotificacionDTO(
            "ACTUALIZACION_STAFF",
            "Se realizo un cambio en el staff: " + staffActulizado.getUsuarioNombre(),
            dto.getUsuarioId()
        ));
        } catch (feign.FeignException e) {
        ...
        return asignacionStaffMapper.toDTO(staffActulizado);
    }
